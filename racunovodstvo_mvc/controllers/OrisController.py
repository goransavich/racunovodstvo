...
class OrisController:
    ...
    def unesi(self, broj_orisa, period):
        # Unos formiranog orisa u tabelu
        try:
            schema = "broj_oris, period"
            value = (broj_orisa, period)
            connection = Database()
            connection.insert(self.tablename, schema, value)
        except Error as e:
            Greske("Greska prilikom unosa broja orisa - OrisController.unesi", e)
    ...
    def postoji_broj_u_orisu(self, broj, duzina):
        try:
            select_columns = "*"
            where_columns = "LEFT (broj_oris, '{}')".format(duzina)
            value = broj
            connection = Database()
    ...
    # provera da li vec postoji oris sa tim mesecom i godinom
    def postoji_broj_u_orisu_mesec_godina(self, broj):
        try:
            select_columns = "*"
            where_columns = "period"
            value = broj
            connection = Database()
            pronadjen_nalog = connection.select_where(self.tablename, select_columns, where_columns, value)
            return pronadjen_nalog
        except Error as e:
            Greske("Pronalazenje u bazi naloga pomocu broja orisa - OrisController.postoji_broj_u_orisu_mesec_godina ", e)
