...
class KnjigaDobavljaca:
    ...
    def __prikaz_podataka_u_tabeli(self, rezultat_stavke):
        ...
        if rezultat_stavke is not None:
            self.tree_tabela_knjiga_dobavljaca.delete(*self.tree_tabela_knjiga_dobavljaca.get_children())
            # Pretraga baze za stavkama naloga
            # Ovde idu podaci iz stavki naloga i prikazuju se u tabeli

            self.tree_tabela_knjiga_dobavljaca.tag_configure('oddrow', background="white")
            self.tree_tabela_knjiga_dobavljaca.tag_configure('evenrow', background="lightblue")
            ...
            for record in konvertovan_rezultat:
                ...
                if count_stavke_naloga % 2 == 0:
                    self.tree_tabela_knjiga_dobavljaca.insert(parent='', index='end', iid=count_stavke_naloga, text='',
    ...
    def __izvuci_pomocnu_knjigu_dobavljaca(self, pocetni_datum, krajnji_datum):
        ...
        rezultat = konto_conn.knjiga_dobavljaca_spisak(pocetni_datum, krajnji_datum)
        return rezultat
    ...
    def prikazi(self):
        ...
        if pocetni > krajnji:
            messagebox.showwarning("Greška", "Početni datum je veći od završnog datuma!", parent=self.prozor_knjiga_dobavljaca)
        elif pocetna_godina != krajnja_godina:
            messagebox.showwarning("Greška", "Pomoćnu knjigu dobavljača možete da dobijete u okviru jedne kalendarske godine!", parent=self.prozor_knjiga_dobavljaca)
        else:
    ...
    def pravljenje_excel_izvestaja(self, podaci, pocetni, krajnji):
        ...
        naslovni_titl = 'KNJIGA DOBAVLJAČA ZA PERIOD ' + pocetni.strftime("%d.%m.%Y.") + ' - ' + krajnji.strftime("%d.%m.%Y.")
        ...
        worksheet.write('A3', 'Dobavljač', zaglavlje)
        ...
        worksheet.autofit()
        workbook.close()
        os.startfile('pomocna_knjiga_dobavljaca_excel.xlsx')

    def exportuj_knjigu_dobavljaca(self):
        try:
            pocetni = self.datum_od.get_date()
            krajnji = self.datum_do.get_date()
    ...
    def __init__(self, master):
        ...
        self.prozor_knjiga_dobavljaca.title("IZVEŠTAJI - Pomoćna knjiga dobavljača")
        self.prozor_knjiga_dobavljaca.resizable(False, False)
        self.prozor_knjiga_dobavljaca.grab_set()
        ...
        self.prozor_knjiga_dobavljaca.geometry("{}x{}+{}+{}".format(window_width, window_height, x_cordinate, y_cordinate))
        self.prozor_knjiga_dobavljaca.columnconfigure(0, weight=1)
        self.prozor_knjiga_dobavljaca.rowconfigure(0, weight=1)
        self.prozor_knjiga_dobavljaca.rowconfigure(1, weight=6)
        self.prozor_knjiga_dobavljaca.rowconfigure(2, weight=1)

        # Prvi frame za unos podataka koji su potrebni za formiranje tabele
        self.prvi_frame = Frame(self.prozor_knjiga_dobavljaca)
        ...
        ##########################################################################################
        # Drugi frame - tabela
        self.drugi_frame = Frame(self.prozor_knjiga_dobavljaca)
        ...
        self.canvas_tabela_knjiga_dobavljaca = Canvas(self.drugi_frame)
        self.canvas_tabela_knjiga_dobavljaca.grid(row=0, column=0, sticky='nsew')
        self.canvas_tabela_knjiga_dobavljaca.columnconfigure(0, weight=1)
        self.canvas_tabela_knjiga_dobavljaca.rowconfigure(0, weight=1)
        ...
        self.tree_tabela_knjiga_dobavljaca = ttk.Treeview(self.canvas_tabela_knjiga_dobavljaca)
        self.tree_tabela_knjiga_dobavljaca.grid(row=0, column=0, padx=10, pady=10, sticky='nsew')
        self.tree_tabela_knjiga_dobavljaca['columns'] = ("Naziv dobavljača", "Početno duguje", "Početno potražuje", "Tekuće duguje", "Tekuće potražuje", "Ukupno duguje", "Ukupno potražuje", "Saldo duguje", "Saldo potražuje")
        self.tree_tabela_knjiga_dobavljaca.column("#0", width=0, stretch=False)
        self.tree_tabela_knjiga_dobavljaca.column("Naziv dobavljača", anchor=tk.W, width=200)
        self.tree_tabela_knjiga_dobavljaca.column("Početno duguje", anchor=tk.E, width=100)
        self.tree_tabela_knjiga_dobavljaca.column("Početno potražuje", anchor=tk.E, width=100)
        self.tree_tabela_knjiga_dobavljaca.column("Tekuće duguje", anchor=tk.E, width=100)
        self.tree_tabela_knjiga_dobavljaca.column("Tekuće potražuje", anchor=tk.E, width=100)
        self.tree_tabela_knjiga_dobavljaca.column("Ukupno duguje", anchor=tk.E, width=100)
        self.tree_tabela_knjiga_dobavljaca.column("Ukupno potražuje", anchor=tk.E, width=100)
        self.tree_tabela_knjiga_dobavljaca.column("Saldo duguje", anchor=tk.E, width=100)
        self.tree_tabela_knjiga_dobavljaca.column("Saldo potražuje", anchor=tk.E, width=100)
        self.treeScroll = ttk.Scrollbar(self.canvas_tabela_knjiga_dobavljaca)
        self.treeScroll.grid(row=0, column=1, sticky='ns')
        self.treeScroll.configure(command=self.tree_tabela_knjiga_dobavljaca.yview)
        self.tree_tabela_knjiga_dobavljaca.configure(yscrollcommand=self.treeScroll.set)

        self.tree_tabela_knjiga_dobavljaca.heading("#0", anchor=tk.W, text="")
        self.tree_tabela_knjiga_dobavljaca.heading("Naziv dobavljača", anchor=tk.CENTER, text="Naziv dobavljača")
        self.tree_tabela_knjiga_dobavljaca.heading("Početno duguje", anchor=tk.CENTER, text="Početno duguje")
        self.tree_tabela_knjiga_dobavljaca.heading("Početno potražuje", anchor=tk.CENTER, text="Početno potražuje")
        self.tree_tabela_knjiga_dobavljaca.heading("Tekuće duguje", anchor=tk.CENTER, text="Tekuće duguje")
        self.tree_tabela_knjiga_dobavljaca.heading("Tekuće potražuje", anchor=tk.CENTER, text="Tekuće potražuje")
        self.tree_tabela_knjiga_dobavljaca.heading("Ukupno duguje", anchor=tk.CENTER, text="Ukupno duguje")
